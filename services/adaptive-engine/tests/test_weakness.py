...
async def client(monkeypatch: pytest.MonkeyPatch) -> AsyncIterator[AsyncClient]:
    monkeypatch.setattr("adaptive_engine.weakness.fetch_topic_catalog", _stub_catalog)
    monkeypatch.setattr("adaptive_engine.weakness.fetch_mastery", _stub_mastery)
    monkeypatch.setattr("adaptive_engine.llm.is_enabled", lambda: False)

    async def _no_llm(**_: Any) -> None:
        return None

    monkeypatch.setattr("adaptive_engine.weakness.llm.call_structured", _no_llm)
...
async def test_weakness_thin_evidence_returns_stub(
    ...
    monkeypatch.setattr("adaptive_engine.weakness.fetch_user_answered_items", _few)
...
async def test_weakness_no_history(
    ...
    monkeypatch.setattr("adaptive_engine.weakness.fetch_user_answered_items", _none)
...
async def test_weakness_enough_evidence_but_llm_off(
    ...
    monkeypatch.setattr("adaptive_engine.weakness.fetch_user_answered_items", _enough)
    r = await client.get("/adaptive/weakness-diagnosis/u-1")
    assert r.status_code == 200
    body = r.json()
    assert body["source"] == "heuristic"
...
async def test_weakness_uses_llm_when_enabled(
    ...
    monkeypatch.setattr("adaptive_engine.weakness.fetch_user_answered_items", _enough)
    monkeypatch.setattr("adaptive_engine.llm.is_enabled", lambda: True)
    monkeypatch.setattr("adaptive_engine.weakness.llm.call_structured", _fake_llm)
