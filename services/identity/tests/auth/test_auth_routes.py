...
from identity.auth.db import sessionmaker
from identity.main import app
from identity.auth.security import hash_otp
...
async def test_verify_otp_publishes_user_created_event(
    ...
    monkeypatch.setattr("identity.auth.routes.publish_user_created", _capture)
...
async def test_email_channel_disabled_skips_smtp_but_otp_row_persists(
    ...
    monkeypatch.setattr("identity.auth.routes.send_otp_email", _no_send)
    ...
    monkeypatch.setattr("identity.auth.routes.flags_client", lambda: _StubClient())
...
async def test_forgot_password_creates_reset_token_for_known_user(
    ...
    monkeypatch.setattr("identity.auth.routes.send_password_reset_email", _capture)
