...
/* ─────────────────────────────────────────────────────────────
   MockTestSparkline — recent attempts polyline + labels.
   Mockup: 612 / 638 / 654 / 670 / 695 / 728 with M-09…M-14.
   ───────────────────────────────────────────────────────────── */

export interface MockTestSparklineProps {
...
export function MockTestSparkline({
  scores,
  max,
  width = 540,
  height = 110,
}: MockTestSparklineProps) {
  if (!scores.length) {
    return <div className="vidya-mocks__empty">No mock attempts yet.</div>;
  }
  const padding = { l: 18, r: 18, t: 8, b: 26 };
  const innerW = width - padding.l - padding.r;
  const innerH = height - padding.t - padding.b;
  const step = scores.length === 1 ? 0 : innerW / (scores.length - 1);
  const min = Math.min(...scores.map((s) => s.value));
  const range = Math.max(1, max - min);
  const pts = scores.map((s, i) => {
    const x = padding.l + step * i;
    const y = padding.t + innerH - ((s.value - min) / range) * innerH;
    return { x, y, ...s };
  });
  const d = pts
  ...
  return (
    <svg
      className="vidya-mocks__chart"
      viewBox={`0 0 ${width} ${height}`}
      role="img"
      aria-label="Recent mock test scores"
    >
      <path
        d={`${d} L${pts[pts.length - 1]!.x.toFixed(1)} ${height - padding.b} L${pts[0]!.x.toFixed(1)} ${height - padding.b} Z`}
        fill="var(--accent)"
        fillOpacity="0.08"
        stroke="none"
      />
